...
function _tapclicks_add_css() {
  ...
  drupal_add_css($theme_path . '/css/tapclicks.css');

  $default_color = variable_get('tapclicks_theme_color');
  if (!empty($default_color)) {
    drupal_add_css($default_color);
  }
}

function tapclicks_preprocess_html(&$variables) {
  ...
  drupal_add_html_head(
      ...
      '#attributes' => array(
          ...
          ), 'tapclicks:viewport_meta'
  );
}

function tapclicks_preprocess_page(&$variables) {
...
function tapclicks_format_comma_field($field_category, $node, $limit = NULL) {
...
function tapclicks_breadcrumb($variables) {
...
function tapclicks_table($variables) {
...
function tapclicks_status_messages(&$variables) {
...
function tapclicks_pager($variables) {
  $tags = $variables['tags'];
...
function tapclicks_tagclouds_weighted(array $vars) {
  $terms = $vars['terms'];
...
function tapclicks_preprocess_node(&$variables) {
  ...
  if (variable_get('node_submitted_' . $node->type, TRUE)) {
    ...
    if (!empty($variables['content']['field_tags'])) {
      $submitted .= '<li>' . tapclicks_format_comma_field('field_tags', $node) . '</li>';
    }
...
function tapclicks_preprocess_views_view_table(&$vars) {
