(function($){Drupal.behaviors.password={attach:function(context,settings){var translate=settings.password;$("input.password-field",context).once("password",function(){var passwordInput=$(this);var innerWrapper=$(this).parent();var outerWrapper=$(this).parent().parent();innerWrapper.addClass("password-parent");$("input.password-confirm",outerWrapper).parent().prepend('<div class="password-confirm">'+translate["confirmTitle"]+" <span></span></div>").addClass("confirm-parent");var confirmInput=$("input.password-confirm",
outerWrapper);var confirmResult=$("div.password-confirm",outerWrapper);var confirmChild=$("span",confirmResult);var passwordMeter='<div class="password-strength"><div class="password-strength-text" aria-live="assertive"></div><div class="password-strength-title">'+translate["strengthTitle"]+'</div><div class="password-indicator"><div class="indicator"></div></div></div>';$(confirmInput).parent().after('<div class="password-suggestions description"></div>');$(innerWrapper).prepend(passwordMeter);var passwordDescription=
$("div.password-suggestions",outerWrapper).hide();var passwordCheck=function(){var result=Drupal.evaluatePasswordStrength(passwordInput.val(),settings.password);if(passwordDescription.html()!=result.message)passwordDescription.html(result.message);if(result.strength==100)passwordDescription.hide();else passwordDescription.show();$(innerWrapper).find(".indicator").css("width",result.strength+"%");$(innerWrapper).find(".password-strength-text").html(result.indicatorText);passwordCheckMatch()};var passwordCheckMatch=
function(){if(confirmInput.val()){var success=passwordInput.val()===confirmInput.val();confirmResult.css({visibility:"visible"});if(this.confirmClass)confirmChild.removeClass(this.confirmClass);var confirmClass=success?"ok":"error";confirmChild.html(translate["confirm"+(success?"Success":"Failure")]).addClass(confirmClass);this.confirmClass=confirmClass}else confirmResult.css({visibility:"hidden"})};passwordInput.keyup(passwordCheck).focus(passwordCheck).blur(passwordCheck);confirmInput.keyup(passwordCheckMatch).blur(passwordCheckMatch)})}};
